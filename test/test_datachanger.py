...
from etl.datachanger import DataChanger
...
class TestGroup:
    ...
    def test_renameColumns(self, spark_configuration):
        ...
        It uses sample data to check if direct pyspark
        methods to **rename columns** have the same output as
        ...
        df = (spark_configuration.createDataFrame
              (data, ['col1', 'col2', 'col3'])
              )
    ...
    def test_filterCountry(self, spark_configuration):
        '''
        function to test static method *filterCountry()* from DataChanger.

        It uses sample data to check wheter direct pyspark methods to
        **filter strings (isin)** have the same output as
        method from custom class.
        ...
        expected_df = df.filter(
            (df['city'].like('Berlin')) | (df['city'].like('Vienna'))
            )
